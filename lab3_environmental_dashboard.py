...
## Keeps visual style consistent (transparent background, minimal grid, compact margins)
PLOT_LAYOUT = dict(
    ...
    xaxis=dict(showgrid=False, tickvals=YEARS),
...
if uploaded is not None:
    ...
    try:
        ...
        ## Display a user-friendly error and stop execution if file is missing
        st.warning("Default file not found. Please upload your Excel dataset in the sidebar.")
        st.stop()
...
if "NordPetro" in company:
    ...
    s12_2023 = s12_vals[-1] or 0
    s3_2023  = s3c11_vals[-1] or 0

    ## Calculate total emissions for 2023, treating missing values as zero
    total_2023 = s12_2023 + s3_2023
    ...
    with c5:
        ...
        for label, val, sub, kind in scorecard:
            color_hex = "#791F1F" if kind == "red" else "#633806"
            st.markdown(f"""
            <div class='metric-card {kind}' style='padding:10px 14px;margin-bottom:6px;'>
              ...
              <div style='font-size:16px;font-weight:700;color:{color_hex};'>{val}</div>
    ...
    with c6:
        ...
        fig = bar_chart(
            YEARS, re_vals, COLORS["green"],
            "Renewable electricity %",
            height=200,
            text=[f"{v:.0f}%" if v is not None else "" for v in re_vals],
        )
    ...
    with c7:
        ...
        fig = bar_chart(
            YEARS, spill_vals, COLORS["amber"],
            "Oil spills (>1 bbl)",
            height=200,
            text=[str(int(v)) if v is not None else "" for v in spill_vals],
        )
    ...
    with c8:
        ...
        fig = bar_chart(
            YEARS, water_vals, "#378ADD",
            "Total water withdrawal (Mm³)",
            height=200,
            text=[f"{v:.1f}" if v is not None else "" for v in water_vals],
        )
    ...
    with c9:
        ...
        fig = bar_chart(
            YEARS, rnd_vals, COLORS["blue"],
            "R&D: low-carbon tech ($M)",
            height=200,
            text=[f"${v:.0f}M" if v is not None else "" for v in rnd_vals],
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    s3_2019 = s3_vals[0] or 0
    s3_2023 = s3_vals[-1] or 0
    ...
    gap_pct = round((proj_2035 - s3_target) / s3_target * 100, 1) if s3_target else 0
    ...
    sbti_gap = (enrol_vals[-1] or 0) - (sbti_vals[-1] or 0)
    ...
    with c2:
        ...
        ## Calculate how much faster the pace needs to be to hit the 2035 target
        pace_needed_pct = round(
            (((s3_2023 - s3_target) / 12) / pace_actual - 1) * 100
        ) if pace_actual else 0
        ...
        kpis = [
            ...
            ("Pace needed vs achieved",  f"+{pace_needed_pct}% faster", "Acceleration needed",          "red"),
        ]

        ## Render each KPI as a compact metric card
        for label, val, sub, kind in kpis:
            color_hex = "#791F1F" if kind == "red" else "#633806" if kind == "amber" else "#0C447C"
            st.markdown(f"""
            <div class='metric-card {kind}' style='padding:10px 14px;margin-bottom:6px;'>
              <div class='metric-label'>{label}</div>
              <div style='font-size:18px;font-weight:700;color:{color_hex};'>{val}</div>
              <div class='metric-sub'>{sub}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with c4:
        ...
        fig_pl = bar_chart(
            YEARS, plast_vals, COLORS["blue"],
            "Own-brand plastic packaging (kt)",
            height=230,
            text=[f"{v:.0f} kt" if v is not None else "" for v in plast_vals],
        )
    ...
    with c6:
        ...
        fig_en = bar_chart(
            YEARS, energy_vals, COLORS["gray"],
            "Total energy consumption (PJ)",
            height=220,
            text=[f"{v:.0f} PJ" if v is not None else "" for v in energy_vals],
        )
